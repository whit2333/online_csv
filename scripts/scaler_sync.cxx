...
void scaler_sync(int RunNumber = 6182, int nevents = 30000) {
  ...
  // Detector tree 
  ROOT::RDataFrame d(*t);//"T", rootfile);
  auto c_n_events_total = d.Count();

  auto d0 = d
  .Define("shmsDC1Planes_large","(P.dc.1x1.nhit + P.dc.1u2.nhit + P.dc.1u1.nhit + P.dc.1v1.nhit + P.dc.1x2.nhit + P.dc.1v2.nhit) > 20")
  .Define("shmsDC2Planes_large","(P.dc.2x1.nhit + P.dc.2u2.nhit + P.dc.2u1.nhit + P.dc.2v1.nhit + P.dc.2x2.nhit + P.dc.2v2.nhit) > 20")
  ...
  h.OnPartialResult(1000, [c, gr](TH1D& h_) {
    ...
    int ipoint = gr->GetN();
    gr->SetPoint(ipoint, ipoint, double(n2) / double(n1 + n2));
  ...
  c->cd();
  h->Clone(); // event loop runs here, this `Draw` is executed after the event loop is finished
