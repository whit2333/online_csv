...
void bsa0(int RunNumber = 6018, int nevents = -1, int prompt = 1, int update = 1, int default_count_goal = 30000) {
  ...
  double theta_shms = j[runnum_str]["spectrometers"]["shms_angle"].get<double>()*M_PI/180.0;

  double P0_hms_setting =
      j[runnum_str]["spectrometers"]["hms_momentum"].get<double>();
  ...
  auto d_hgc_cut =
      d2_coin
          .Define("p_eprime", [=](double dp) { return P0_shms * (100.0 + dp) / 100.0; }, {"H.gtr.dp"})
          .Define("th_eprime", [=](double th) { return theta_shms + th; }, {"H.gtr.th"})
          .Define("ph_eprime", [=](double ph) { return ph; }, {"H.gtr.ph"})
          .Define("P_eprime_vec",
                  [=](double p, double th, double ph) {
                    Polar3D v(p, th, ph);
                    //std::cout << " theta "  << th*180.0/M_PI << "\n";
                    return v;
                  }, {"p_eprime", "th_eprime", "ph_eprime"})
          .Define("l2",[=](const Polar3D& v) { 
                  XYZTVector l2(v.X(), v.Y(), v.Z(), v.R());
                  //std::cout << l2.Theta()*180.0/M_PI;
                  return l2;
          },{"P_eprime_vec"})
          .Define("q",[=](const XYZTVector& v) {
            XYZTVector res =  XYZTVector(0,0,10.6,10.6)- v;
            std::cout << res.Theta()*180.0/M_PI << "\n";;
            return res; },{"l2"})
          .Define("theta_q_deg",[=](const XYZTVector& v) { return v.Theta()*180.0/M_PI;},{"q"})
          .Define("nu",[=](const XYZTVector& q) { return q.E(); },{"q"})
          .Define("Q2",[=](const XYZTVector& v) { return 1.0*(v.Dot(v)); },{"q"})
          .Define("x_bj",[=](double Q2, double nu) { return Q2/(2.0*0.938*nu); },{"Q2","nu"})
          .Define("p_had", [=](double dp) { return P0_shms * (100.0 + dp) / 100.0; }, {"P.gtr.dp"})
          .Define("th_had", [=](double th) { return theta_shms + th; }, {"P.gtr.th"})
          .Define("ph_had", [=](double ph) { return ph; }, {"P.gtr.ph"})
          .Define("P_had_vec",
                  [=](double p, double th, double ph) {
                    Polar3D v(p, th, ph+M_PI);
                    return v;
                  },
                  {"p_had", "th_had", "ph_had"})
          .Define("p2",[=](const Polar3D& v) { 
                  XYZTVector p(v.X(), v.Y(), v.Z(), v.R());
                  return p;
          },{"P_had_vec"})
          .Filter(
          ...
          .Define("y", [=](double nu){return nu/Pbeam.E();},{"nu"})
          .Define("z", [](double nu, double p_had){return std::sqrt(0.135*0.135 +p_had*p_had)/nu;},{"nu","p_had"})
          .Define("gamma",[](double x, double Q2) { return 2.0 * x * 0.938 / std::sqrt(Q2);},{"x_bj","Q2"})
          .Define("epsilon",[](double y, double gamma) {
             return (1.0-y-y*y*gamma*gamma/4.0)/(1.0 - y + y * y / 2.0 + y * y * gamma * gamma / 4.0);
          },{"y","gamma"})
          .Define("P_h_perp_vec",[](const XYZTVector& q,const XYZTVector& p2){
            auto qdir = q.Vect().Unit();
            //std::cout << " qdir : " << qdir.Theta() << ", " << qdir.Phi() << "\n";
            //std::cout << " p2   : " << p2.Theta() << ", " << p2.Phi() << "\n";
            auto res = (p2.Vect() - qdir.Dot(p2.Vect()) * qdir);
            //std::cout << " perp : " << res.Theta() << ", " << res.Phi() << "\n";
            //std::cout << res << "\n";
            return res;
          },{"q","p2"})
          .Define("P_h_perp",[](const Pvec3D& p){ return p.R();},{"P_h_perp_vec"})
          .Define("phi_ph",[=](const XYZTVector& q, const Pvec3D& p){ 
            Pvec3D u = (q.Vect().Cross(Pbeam.Vect())).Unit();
            double sign = (q.Vect().Dot(Pbeam.Vect()));
            sign = sign/std::abs(sign);
            double res = sign*ROOT::Math::VectorUtil::Angle(u,p);
            return res;
           ;},{"q","P_h_perp_vec"})
      //W2      = Mp2 + 2.0*Mp*nu - Q2;
  ...
  auto h_phi_ph  = d_hgc_cut.Histo1D({"h_phi_ph ","; ph",100,-3.2,3.2}, "phi_ph");
  auto h_theta_q = d_hgc_cut.Histo1D({"h_theta_q ","; theta_q [degrees]",360,-180.0,180.0}, "theta_q_deg");

  auto d_kine =
      d_hgc_cut.Filter([](const Polar3D& pvec) { return pvec.R() > 1.0; }, {"P_had_vec"});

  auto d_hel_1 = d_hgc_cut.Filter("T.helicity.hel>0");
  auto d_hel_2 = d_hgc_cut.Filter("T.helicity.hel<0");

  auto c_hel_1 = d_hel_1.Count();
  auto c_hel_2 = d_hel_2.Count();

  std::cout << *(d_kine.Count()) << "\n";

  std::cout << *c_hel_1 << "\n";
  std::cout << *c_hel_2 << "\n";

  double asym = (double(*c_hel_1)- double(*c_hel_2))/(double(*c_hel_1)+ double(*c_hel_2));
  std::cout << " asym = " << asym  << "\n";
