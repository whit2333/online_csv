...
void good_coin_counter(int RunNumber = 6018, int nevents = -1, int prompt = 0, int update = 1,
  ...
  if (j.find(runnum_str) == j.end()) {
    std::cout << "Run " << RunNumber << " not found in db2/run_list.json\n";
  ...
  auto c_n_events_coin = d.Filter("(fEvtHdr.fEvtType == 6) || (fEvtHdr.fEvtType == 4)").Count();
  ...
  auto d0 = d.Define("hms_e_EoverP",
                ...
                .Define("hms_e_EoverP_nGood", [](doublers& EOverP) { return (int)EOverP.size(); },
                        {"hms_e_EoverP"})
                .Define("shms_pi_preshower", [](doublers& EOverP) { return EOverP[EOverP < 0.3]; },
                        {"P.cal.eprtracknorm"})
                .Define("shms_pi_preshower_nGood",
                        [](doublers& EOverP) { return (int)EOverP.size(); }, {"shms_pi_preshower"});

  auto d2 = d0.Filter([](double npe) { return npe > 1.0; }, {"H.cer.npeSum"});
  auto d3 = d2.Filter([](int n_good) { return n_good > 0; }, {"shms_pi_preshower_nGood"});

  auto d_spec_cuts = d2.Filter(hpdelta).Filter(epiCut);

  // Find the coin peak
  auto h_coin_time =
      d_spec_cuts.Histo1D({"coin_time", "coin_time", 800, 0, 100}, "CTime.ePiCoinTime_ROC2");
  ...
  auto d_coin_time_cut = d_spec_cuts.Filter(
      [=](double coin_time) { return std::abs(coin_time - coin_peak_center) < 2.0; },
      {"CTime.ePiCoinTime_ROC2"});
  auto h_coin_time2 =
      d_coin_time_cut.Histo1D({"coin_time2", "coin_time2", 800, 0, 100}, "CTime.ePiCoinTime_ROC2");

  auto d_random = d_spec_cuts.Filter(
      [=](double coin_time) { return std::abs(coin_time - coin_peak_center - 28) < 10.0; },
      {"CTime.ePiCoinTime_ROC2"});
  auto h_coin_time3 =
      d_random.Histo1D({"coin_time2", "coin_time2", 800, 0, 100}, "CTime.ePiCoinTime_ROC2");
  ...
  auto h_EOverP_0 = d0.Histo1D<doublers>(
      {"hms_e_EoverP_0", "SHMS total shower; SHMS E/P", 100, 0.05, 1.8}, "P.cal.etottracknorm");
  auto h_EOverP_2 = d2.Histo1D<doublers>(
      {"hms_e_EoverP_2", "SHMS total shower; SHMS E/P", 100, 0.05, 1.8}, "P.cal.etottracknorm");
  auto h_EOverP_3 = d3.Histo1D<doublers>(
      {"hms_e_EoverP_3", "SHMS total shower; SHMS E/P", 100, 0.05, 1.8}, "P.cal.etottracknorm");

  auto h_EprOverP_0 = d0.Histo1D<doublers>(
      {"hms_e_EoverP_0", "SHMS pre-shower; SHMS E/P", 100, 0.05, 1.8}, "P.cal.eprtracknorm");
  auto h_EprOverP_2 = d2.Histo1D<doublers>(
      {"hms_e_EoverP_2", "SHMS pre-shower; SHMS E/P", 100, 0.05, 1.8}, "P.cal.eprtracknorm");
  auto h_EprOverP_3 = d3.Histo1D<doublers>(
      {"hms_e_EoverP_3", "SHMS pre-shower; SHMS E/P", 100, 0.05, 1.8}, "P.cal.eprtracknorm");
  ...
  json jruns;
  {
    std::ifstream input_file("db2/coin_run_count_list.json");
    try {
      input_file >> jruns;
    } catch (json::parse_error) {
  ...
  jruns[run_str]["total trigger events"] = int(*c_n_events_total);
  jruns[run_str]["coin trigger events"]  = int(*c_n_events_coin);
  jruns[run_str]["pi+K counts"]          = int(*coin_counts);
  jruns[run_str]["random background"]    = double(random_bg);
  jruns[run_str]["pion counts"]          = int(*pion_count);
  jruns[run_str]["pi/K ratio"]           = pi_K_ratio;
  jruns[run_str]["kaon counts"]          = kaon_counts;
  jruns[run_str]["pion bg sub. counts"]  = pion_corrected;
  jruns[run_str]["kaon bg sub. counts"] =
      double(double(kaon_counts) - random_bg * kaon_counts / (double(*coin_counts)));
  // std::cout << std::setw(4) << j_current_run << "\n";
  // jruns[run_str] = nlohmann::json::parse(j_current_run.dump());

  // jruns[run_str] = nlohmann::json::parse(j_current_run.dump());
  if (update) {
    std::ofstream json_output_file("db2/coin_run_count_list.json");
  ...
  gSystem->mkdir("results/good_coin_counter", true);
  c->SaveAs((std::string("results/good_coin_counter/c1_") + std::to_string(RunNumber) + ".pdf").c_str());
  c->SaveAs((std::string("results/good_coin_counter/c1_") + std::to_string(RunNumber) + ".png").c_str());
  ...
  c->SaveAs((std::string("results/good_coin_counter/c2_") + std::to_string(RunNumber) + ".pdf").c_str());
  c->SaveAs((std::string("results/good_coin_counter/c2_") + std::to_string(RunNumber) + ".png").c_str());
}
