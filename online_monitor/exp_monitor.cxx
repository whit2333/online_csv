...
void exp_monitor(Int_t RunNumber = 6216, Int_t MaxEvent = 500000) {
  ...
  auto exp_monitor = new hallc::MonitoringDisplay(RunNumber);
  exp_monitor->_data._root_folder = "/exp/";

  auto plt1 = exp_monitor->CreateDisplayPlot(
      "hms/", "Q2",
      [&](hallc::DisplayPlot& plt) {
        plt._plot_data._canvas = new TCanvas(plt.GetName().c_str(), plt.GetName().c_str());
        plt._plot_data._hists1 = { new TH1F("h_Q2", "h_Q2", 100, 0, 9)};
      ...
      [&](hallc::DisplayPlot& plt) {
        // is good?
        plt._plot_data._hists1[0]->Fill(hkin_primary->GetQ2());
        //plt._plot_data._hists1[1]->Fill(shmsDC2Planes_nhits);
        return 0;
      });

  auto kine_mon                = new hallc::ExperimentMonitorPostProcess(exp_monitor);
  kine_mon->_analyzer          = analyzer;
  kine_mon->_hod               = hhod;
  kine_mon->_hgcer             = hcer;
  kine_mon->_dc                = hdc;
  kine_mon->_spectrometer_name = "asdf";

  //auto shms_mon_pp                = new hallc::ExperimentMonitorPostProcess(exp_monitor);
  ...
  analyzer->AddPostProcess(kine_mon);
