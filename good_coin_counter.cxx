...
void good_coin_counter(int RunNumber = 6018, int nevents = -1, const char* codatype = "COIN") {
  ...
  std::string epiCut  = "P.aero.npeSum > 1.0 && P.cal.eprtracknorm < 0.4 "
                        "&& H.cer.npeSum > 1.0 && H.cal.etottracknorm > 0.6 "
                        "&& H.cal.etottracknorm < 2.0"; 
  ...
  auto d2 = d0
    ...
    //.Filter(
    //  [](int n_good) { return  n_good >0; },{"hms_e_EoverP_nGood"})
    ;
  ...
  j_current_run["pion bg sub. counts"] = double(double(*pion_count) - random_bg*double(*pion_count)/(double(*coin_counts))); 
  j_current_run["kaon bg sub. counts"] = double(double(kaon_counts) - random_bg*kaon_counts/(double(*coin_counts))); 
